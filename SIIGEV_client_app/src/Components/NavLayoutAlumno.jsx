...
function NavLayoutAlumno({children}) {
    return (
        ...
        <div className="bg-light border-end d-flex flex-column h-100" style={{width: "300px"}}>
            <ul className="list-unstyled">
                <li className="p-3 border-bottom">
                    <Link to="/a/home" className="link-secondary link-underline link-underline-opacity-0 link-underline-opacity-100-hover"> {/* Cambiado a /a/home */}
                ...
                {clases.length > 0 ? (
                    clases.map((clase) => (
                        <ClassItem key={clase.id} claseNombre={clase.nombre} href={`/a/clase/${clase.id}`} />
                    ))
                ) : (
...
export default NavLayoutAlumno;